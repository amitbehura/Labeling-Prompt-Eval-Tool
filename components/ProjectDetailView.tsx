...
import { Project, Dataset, EvaluationRun, GeminiModel } from '../types';
...
export const ProjectDetailView: React.FC<ProjectDetailViewProps> = ({ project, dataset, onBack, onUpdateProject }) => {
  // Configuration State
  const [systemPrompt, setSystemPrompt] = useState(project.systemPrompt || "You are a helpful AI assistant.");
  const [selectedModel, setSelectedModel] = useState<string>(project.model || GeminiModel.FLASH);
  const [inputColumn, setInputColumn] = useState<string>(project.columnMapping?.input || dataset.columns[0] || '');
  const [expectedOutputColumn, setExpectedOutputColumn] = useState<string>(project.columnMapping?.expectedOutput || dataset.columns[1] || '');
  ...
  // Initialize Runs from Dataset if empty
  // We explicitly map on mount to ensure data is visible immediately
  useEffect(() => {
    if (runs.length === 0 && dataset.content.length > 0) {
      const initialRuns: EvaluationRun[] = dataset.content.map((row, index) => ({
        id: `run-${index}`,
        evaluationId: 'playground',
  ...
  // Handle Input Mapping Changes (Updates the 'userInput' and 'expectedOutput' in idle runs)
  const handleMappingChange = (type: 'input' | 'expected', column: string) => {
    ...
    // Force update table rows for rows that haven't run yet
    setRuns(prev => prev.map((run, idx) => {
      // We allow updating even if not idle to correct data display errors, 
      // but typically we only want to change inputs for new runs. 
      // Given user feedback "Parsing is not happening", we will be aggressive here.
  ...
  const handleRunEvaluation = async () => {
    ...
    for (let i = 0; i < newRuns.length; i++) {
        ...
        try {
            const result = await generateWithGemini(systemPrompt, newRuns[i].userInput, selectedModel, categories);
            
            // Exact Match Logic
            const generated = result.output.trim();
            const expected = newRuns[i].expectedOutput.trim();
            // If categories are used, exact match is case-sensitive typically, but we'll be safe
    ...
    onUpdateProject({
        ...
        avgLatency: Number((totalLatency / newRuns.length / 1000).toFixed(2)),
